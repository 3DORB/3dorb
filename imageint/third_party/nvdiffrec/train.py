...
def validate(glctx, geometry, opt_material, lgt, dataset_validate, out_dir, FLAGS):
    ...
    with open(os.path.join(out_dir, 'metrics.txt'), 'w') as fout:
        ...
        if os.getenv('NVDIFFREC_GT_ENV_MAP') == '1':
            ...
            frame_iter = iter(test_frames)

        print("Running validation")
        for it, target in enumerate(dataloader_validate):
            if os.getenv('NVDIFFREC_GT_ENV_MAP') == '1':
                frame = next(frame_iter)
                if 'scene_name' in frame and os.environ['NVDIFFREC_LIGHT_SCENE'] != frame['scene_name']:
                    continue
                w2c = torch.linalg.inv(torch.tensor(frame['transform_matrix'], dtype=torch.float32))
                w2c = w2c @ util.rotate_x(-np.pi / 2)
                c2w = torch.linalg.inv(w2c).numpy()
                arr = os.path.join(dataset_validate.base_dir.replace(scene, os.environ['NVDIFFREC_LIGHT_SCENE']), 'env_map', frame['file_path'].split('/')[-1]) + '.exr'
                print('Loading light from', arr)
                arr = load_rgb_exr(arr)
                arr = env_map_to_cam_to_world_by_convention(arr, c2w, 'ours')
                latlong_img = torch.tensor(arr, dtype=torch.float32, device='cuda')
                cubemap = util.latlong_to_cubemap(latlong_img, [512, 512])
                lgt = light.EnvironmentLight(cubemap)
                lgt.build_mips()
        ...
        print('nvdiffrec mse', mse_values)
        print('nvdiffrec psnr', psnr_values)
...
if __name__ == "__main__":
    ...
    if os.path.splitext(FLAGS.ref_mesh)[1] == '.obj':
        ...
        if os.path.isdir(os.path.join(FLAGS.ref_mesh, 'train_mask')):
            dataset_train = DatasetCapture(os.path.join(FLAGS.ref_mesh, 'transforms_train.json'), FLAGS, examples=(FLAGS.iter + 1) * FLAGS.batch)
            dataset_validate = DatasetCapture(os.path.join(FLAGS.ref_mesh, 'transforms_test.json'), FLAGS)
        elif os.path.isfile(os.path.join(FLAGS.ref_mesh, 'poses_bounds.npy')):
