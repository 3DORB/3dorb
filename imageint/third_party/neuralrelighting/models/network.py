...
class envmapInitial(nn.Module):
    ...
    def forward(self, x):
        ...
        # FIXME hack to circumvent the error with spatial dim = 1
        # https://github.com/pytorch/pytorch/issues/45687
        x = self.conv(x)
        x = torch.instance_norm(
            x, self.bn.weight, self.bn.bias,
            self.bn.running_mean, self.bn.running_var, not self.bn.track_running_stats, self.bn.momentum, self.bn.eps,
