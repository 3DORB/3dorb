...
def evaluate(**kwargs):
    ...
    if eval_rendering:
        ...
        for data_index, (indices, model_input, ground_truth) in enumerate(eval_dataloader):
            ...
            img.save('{0}/gt_{1}.png'.format(images_dir,'%03d' % indices[0]))
            ...
            pyexr.write('{0}/normal_{1}.exr'.format(images_dir,'%03d' % indices[0]), normal)
            ...
            pyexr.write('{0}/depth_{1}.exr'.format(images_dir,'%03d' % indices[0]), depth)
            np.save('{0}/depth_{1}.npy'.format(images_dir,'%03d' % indices[0]), depth)
            ...
            img.save('{0}/depth_{1}.png'.format(images_dir,'%03d' % indices[0]))

            rgb_eval_masked = rgb_eval * mask
            rgb_gt_masked = rgb_gt * mask

            img = Image.fromarray(((rgb_eval_masked) * 255).astype(np.uint8))
            img.save('{0}/eval_with_gt_mask_{1}.png'.format(images_dir,'%03d' % indices[0]))
            img = Image.fromarray(((rgb_gt_masked) * 255).astype(np.uint8))
            img.save('{0}/gt_with_gt_mask_{1}.png'.format(images_dir,'%03d' % indices[0]))
