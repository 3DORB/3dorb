...
def evaluate(**kwargs):
    ...
    # extract mesh
    if False and (not edit_diffuse) and (not relight) and eval_dataset.has_groundtruth:
        with torch.no_grad():
    ...
    for data_index, (indices, model_input, ground_truth) in enumerate(eval_dataloader):
        ...
        if not relight:
            ...
            depth_vis = ((depth - depth_valid.min().item()) / (depth_valid.max().item() - depth_valid.min().item())).clip(0,
                                                                                                                          1)
